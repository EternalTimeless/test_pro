  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", AudioManager = class AudioManager {
        ...
        configureConcurrent(maxPlaysPerWindow, playWindow) {
          if (maxPlaysPerWindow === void 0) {
            maxPlaysPerWindow = 3;
          }

          if (playWindow === void 0) {
            playWindow = 0.05;
          }
        ...
        preload(sound, onComplete) {
          ...
          var cachedClip = this._clipCache.get(sound);
          ...
          if (onComplete) {
            var callbacks = this._loadingCallbackMap.get(sound);
        ...
        _updateWindowState(now) {
          var cutoff = now - this._playWindow;

          while (this._ringCount > 0) {
            var idx = (this._ringHead - this._ringCount + this._maxPlaysPerWindow) % this._maxPlaysPerWindow;
        ...
        _canPlaySound(key, cooldown) {
          var now = Date.now() / 1000; // 饱和标记检查必须放在 _updateWindowState 之前，但饱和过期判断必须在 return false 之前
          ...
          if (cooldown > 0) {
            var lastTime = this._lastPlayTimeMap.get(key);
        ...
        playOneShot(sound, volume, cooldown) {
          if (volume === void 0) {
            volume = 1.0;
          }
          ...
          var key = this._getSoundKey(sound); // 并发控制检查
          ...
          if (sound instanceof AudioClip) {
            ...
            var _clip = this._clipCache.get(sound);

            if (_clip) {
              this._audioSource.playOneShot(_clip, volume);
