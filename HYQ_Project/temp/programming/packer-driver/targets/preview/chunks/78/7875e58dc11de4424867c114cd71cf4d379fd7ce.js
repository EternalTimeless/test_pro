    ...
    execute: function () {
      ...
      _export("Role", Role = (_dec = ccclass('Role'), _dec2 = property({
        ...
        setEntryWeaponVisible(visible) {
          ...
          var socket = this.findAncestorByName(this.arms, Role.propSocketNodeName);
        ...
        setShadowCastingEnabled(enabled) {
          ...
          for (var i = 0; i < this.shadowRenderers.length; i++) {
            var renderer = this.shadowRenderers[i];
        ...
        restoreInitialArmsChildTransforms() {
          for (var i = 0; i < this.initialArmsChildTransforms.length; i++) {
            var _item$node, _item$parent;

            var item = this.initialArmsChildTransforms[i];
        ...
        attackEvent(num, visualBulletCount, damageScale, lockWorldX, playEffect, initialBulletRandomX) {
          ...
          if (lockWorldX === void 0) {
            lockWorldX = this.node.worldPosition.x;
          }
          ...
          if (initialBulletRandomX === void 0) {
          ...
          if (mergeVisualBullets) {
            ...
            var _lockTarget = Role.getLockableLalianTarget(bullet, lockWorldX);

            Role.aimBulletToTarget(bullet, _lockTarget);
          }
          ...
          for (var i = 1; i < visualBulletCount; i++) {
            ...
            _bullet.node.setWorldPosition(pos);

            var x = (Math.random() - 0.5) * 2;
            _bullet.node.x += x;

            var _lockTarget2 = Role.getLockableLalianTarget(_bullet, lockWorldX);
            ...
            Role.aimBulletToTarget(_bullet, _lockTarget2);
        ...
        static aimBulletToCurrentTarget(bullet, lockWorldX) {
          if (lockWorldX === void 0) {
            lockWorldX = bullet.node.worldPosition.x;
          }
        ...
        static aimBulletToTarget(bullet, target) {
          ...
          var aimPos = target.getLockAimWorldPosition ? target.getLockAimWorldPosition(bullet.node.worldPosition, Role.aimVector) : target.hitNode.worldPosition;
