    ...
    execute: function () {
      ...
      _export("MonsterBattleTaerget", MonsterBattleTaerget = (_dec = ccclass('MonsterBattleTaerget'), _dec2 = property({
        ...
        applyDeathAnimYOffset() {
          ...
          this.fbx.node.y = this.fbxOriginY;
          this.cacheNormalDieGroundMinWorldY();
          var progress = Math.max(0, Math.min(1, this.normalDieAnimElapsed / this.normalDieAnimDuration));
          var peakWeight = Math.sin(progress * Math.PI);
          var desiredReduce = this.smallMonsterDeathAnimPeakReduce * peakWeight;
          var currentLift = Math.max(0, this.getDeathMeshMinWorldY() - this.normalDieGroundMinWorldY);
          var actualReduce = Math.min(desiredReduce, currentLift);
        ...
        getDeathMeshMinWorldY() {
          ...
          for (var i = 0; i < this.deathBoundRenderers.length; i++) {
            var _renderer$model;

            var renderer = this.deathBoundRenderers[i];

            if (!(renderer != null && renderer.isValid) || !renderer.enabled) {
              continue;
            }

            var bounds = (_renderer$model = renderer.model) == null ? void 0 : _renderer$model.worldBounds;
