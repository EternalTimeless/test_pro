    ...
    execute: function () {
      ...
      _export("FlayPropManager", FlayPropManager = (_dec = ccclass('FlayPropManager'), _dec(_class = (_class2 = class FlayPropManager extends (_crd && UnityUpComponent === void 0 ? (_reportPossibleCrUseOfUnityUpComponent({
        ...
        _update(deltaTime) {
          ...
          var propList = (_crd && PropManager === void 0 ? (_reportPossibleCrUseOfPropManager({
            error: Error()
          ...
          for (var i = propList.length - 1; i >= 0; i--) {
            ...
            for (var j = 0; j < this._IfPList.length; j++) {
              var element = this._IfPList[j];
              var rameove = element.flyProp(prop);
        ...
        shuffleArray(array) {
          var shuffled = array; // 创建数组副本（避免修改原数组）

          for (var i = shuffled.length - 1; i > 0; i--) {
            var j = Math.floor(Math.random() * (i + 1)); // 生成随机索引 [0, i]
        ...
        addIFlay(IfP) {
          var index = this._IfPList.indexOf(IfP);
...
//# sourceMappingURL=80f420cd19a5967732f7f177a68c3670990311d3.js.map