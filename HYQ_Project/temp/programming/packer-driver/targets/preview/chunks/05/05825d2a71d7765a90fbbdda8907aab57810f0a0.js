  ...
  return {
    ...
    execute: function () {
      ...
      _export("AttackParkPlay", AttackParkPlay = (_dec = ccclass('AttackParkPlay'), _dec(_class = class AttackParkPlay extends Component {
        ...
        onLoad() {
          var prat = this.node.getComponent(ParticleSystem);
          this.pratList.push(prat);
          var count = this.node.children.length;

          for (var i = 0; i < count; i++) {
            var _prat = this.node.children[i].getComponent(ParticleSystem);

            this.pratList.push(_prat);
          }
        }

        play() {
          for (var i = 0; i < this.pratList.length; i++) {
...
//# sourceMappingURL=05825d2a71d7765a90fbbdda8907aab57810f0a0.js.map