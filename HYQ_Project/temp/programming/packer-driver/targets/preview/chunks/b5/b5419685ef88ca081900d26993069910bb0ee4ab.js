  ...
  return {
    ...
    execute: function () {
      ...
      _export("PerformanceBlockHUD", PerformanceBlockHUD = (_dec = ccclass('PerformanceBlockHUD'), _dec2 = property({
        ...
        onBeforeUpdate() {
          var now = this.now(); // A sample remains active for one complete frame, including lateUpdate and AFTER_UPDATE callbacks.
        ...
        refreshHud() {
          ...
          var scanStart = this.now();
          var blockStats = this.scanScene();
          var scanMs = this.now() - scanStart;
          var fps = this.frameTime > 0 ? this.frameCount / this.frameTime : 0;
          var frameMs = this.frameCount > 0 ? this.frameTime * 1000 / this.frameCount : 0;
          this.frameCount = 0;
          this.frameTime = 0;
          var device = (_director$root = director.root) == null ? void 0 : _director$root.device;
          var drawCalls = (_device$numDrawCalls = device == null ? void 0 : device.numDrawCalls) != null ? _device$numDrawCalls : 0;
          var triangles = (_device$numTris = device == null ? void 0 : device.numTris) != null ? _device$numTris : 0;
          var instances = (_device$numInstances = device == null ? void 0 : device.numInstances) != null ? _device$numInstances : 0;
          var lines = [];
          lines.push("[\u6027\u80FD\u5206\u5757] FPS " + fps.toFixed(1) + " | \u5E27 " + frameMs.toFixed(2) + "ms | DC " + drawCalls + " | \u4E09\u89D2 " + this.formatNumber(triangles) + " | \u5B9E\u4F8B " + instances);
          var heapText = this.getHeapText();
          ...
          if (topClasses.length > 0) {
            ...
            for (var _i = 0; _i < topClasses.length; _i++) {
              var item = topClasses[_i];
              lines.push("  " + (_i + 1) + ". " + item.name + " " + item.ms.toFixed(2) + "ms");
            }
          }
        ...
        scanScene() {
          var stats = this.createBlockStatsMap();
          ...
          while (this.scanStack.length > 0) {
            var node = this.scanStack.pop();

            if (!node || !node.isValid || !node.activeInHierarchy) {
              continue;
            }

            var _block2 = this.classifyNode(node);

            var blockStat = stats[_block2];
            blockStat.nodes++;
            ...
            for (var _i2 = 0; _i2 < node.children.length; _i2++) {
              this.scanStack.push(node.children[_i2]);
            }
          }
        ...
        addRendererStats(stats, renderer) {
          ...
          stats.renderers++;
          var drawCount = this.getRendererDrawCount(renderer);
          stats.estimatedMainDc += drawCount;
          var shadowCastingMode = (_shadowCastingMode = renderer.shadowCastingMode) != null ? _shadowCastingMode : 0;
        ...
        getRendererDrawCount(renderer) {
          ...
          var subModels = (_model = renderer.model) == null ? void 0 : _model.subModels;
        ...
        classifyNode(node) {
          ...
          var cached = this.nodeBlockCache.get(node);
          ...
          while (current) {
            if (current !== node) {
              var ancestorBlock = this.nodeBlockCache.get(current);
              ...
              if (ancestorBlock) {
                ...
                var inheritedBlock = sawParticle || ancestorBlock === '特效' ? '特效' : sawUi || ancestorBlock === 'UI' ? 'UI' : '环境';
            ...
            for (var i = 0; i < components.length; i++) {
              ...
              var _className2 = js.getClassName(component) || ((_component$constructo2 = component.constructor) == null ? void 0 : _component$constructo2.name) || '';

              var explicitBlock = this.getExplicitBlockByClassName(_className2);
            ...
            var nameBlock = this.getBlockByNodeName(current.name);
        ...
        createHud() {
          var scene = director.getScene();
          var canvas = scene == null ? void 0 : scene.getComponentInChildren(Canvas);
          ...
          var hudNode = new Node('PerformanceBlockHUD_Label');
          hudNode.layer = Layers.Enum.UI_2D;
          canvas.node.addChild(hudNode);
          var transform = hudNode.addComponent(UITransform);
          transform.setContentSize(Math.max(320, this.panelWidth), Math.max(240, this.panelHeight));
          var alignRight = this.panelPosition === HudPanelPosition.右上角;
          transform.setAnchorPoint(alignRight ? 1 : 0, 1);
          var widget = hudNode.addComponent(Widget);
          ...
          var label = hudNode.addComponent(Label);
          ...
          var outline = hudNode.addComponent(LabelOutline);
        ...
        applyVisualHide(component, block) {
          ...
          var record = this.hiddenVisualMap.get(component);
        ...
        syncHiddenVisuals() {
          for (var i = this.hiddenVisualRecords.length - 1; i >= 0; i--) {
        ...
        collectKnownHookTargets() {
          ...
          for (var i = 0; i < knownHooks.length; i++) {
            var item = knownHooks[i];
            var ctor = js.getClassByName(item.className);
        ...
        addHookTarget(prototype, methodName, fixedBlock) {
          var method = prototype == null ? void 0 : prototype[methodName];

          if (typeof method !== 'function') {
            return;
          }

          for (var i = 0; i < this.hookTargets.length; i++) {
            var target = this.hookTargets[i];
        ...
        installHook(prototype, methodName, fixedBlock) {
          var original = prototype == null ? void 0 : prototype[methodName];

          if (typeof original !== 'function') {
            return;
          }

          for (var i = 0; i < this.hookRecords.length; i++) {
            var record = this.hookRecords[i];
        ...
        restoreHooks() {
          for (var i = this.hookRecords.length - 1; i >= 0; i--) {
        ...
        getHeapText() {
          ...
          var memory = perf == null ? void 0 : perf.memory;
          ...
          var total = memory.totalJSHeapSize / 1024 / 1024;
