  ...
  return {
    ...
    execute: function () {
      ...
      _export("JumpParabola", JumpParabola = class JumpParabola extends (_crd && JumpSequenceBase === void 0 ? (_reportPossibleCrUseOfJumpSequenceBase({
        ...
        move(dt) {
          // 速度控制
          var t = dt * this.jumpSpeed * 2.5; // 应用速度曲线调制
          ...
          var pos = this.calculatePosition(this._time);
        ...
        calculatePosition(t) {
          // X 轴线性插值（2D/3D 都需要）
          var x = this.startPos.x + (this.endPos.x - this.startPos.x) * t; // Z 轴线性插值（2D 场景下 Z 始终为 0，3D 场景正常插值）

          var z = this.startPos.z + (this.endPos.z - this.startPos.z) * t; // Y 轴基础线性插值

          var baseY = this.startPos.y + (this.endPos.y - this.startPos.y) * t; // 抛物线偏移：-4h(t-0.5)² + h
          // 这个公式保证：
          // - t=0: offset = -4h×0.25 + h = 0 (起点无偏移)
          // - t=0.5: offset = -4h×0 + h = h (中点最高)
          // - t=1: offset = -4h×0.25 + h = 0 (终点无偏移)

          var parabola = -4 * this.jumpHeight * Math.pow(t - 0.5, 2) + this.jumpHeight; // 额外的曲线调制（可选）
          ...
          if (this.curveRange) {
            ...
            var cy = this.curveRange.curve.evaluate(t) * scaleMultiplier; // 中间时刻影响最大，两端影响最小

            var scale = 1 - Math.abs(t - 0.5) / 0.5;
            parabola += cy * scale;
          }

          var y = baseY + parabola;
...
//# sourceMappingURL=b6919de7425c243c4e1697f376cb4f6f6b2769a4.js.map