  ...
  return {
    ...
    execute: function () {
      ...
      _export("JumpScatter", JumpScatter = class JumpScatter extends (_crd && JumpSequenceBase === void 0 ? (_reportPossibleCrUseOfJumpSequenceBase({
        ...
        init(jumpNode, endPos, jumpHeight, jumpSpeed, scatterAngle) {
          if (scatterAngle === void 0) {
            scatterAngle = 0;
          }
        ...
        generateScatterPath() {
          ...
          direction.normalize();
          var perpendicular = this.getPerpendicularVector(direction);
          var p1 = this.createControlPoint(this.startPos, direction, perpendicular, totalDistance * 0.33, this.scatterRadius * 1.2, this.jumpHeight * 0.7);
          this.controlPoints.push(p1);
          var p2 = this.createControlPoint(this.startPos, direction, perpendicular, totalDistance * 0.67, this.scatterRadius * 0.8, this.jumpHeight * 1.0);
          this.controlPoints.push(p2);
          this.controlPoints.push(this.endPos.clone());
        }

        createControlPoint(basePos, direction, perpendicular, forwardDist, scatterDist, heightOffset) {
          var point = basePos.clone();
          var forward = direction.clone().multiplyScalar(forwardDist);
          point.add(forward);

          if (scatterDist > 0) {
            var radAngle = math.toRadian(this.scatterAngle);
            var scatterOffset = perpendicular.clone();
            ...
            if ((_crd && LayerManager === void 0 ? (_reportPossibleCrUseOfLayerManager({
              ...
              var _cos = Math.cos(radAngle);

              var _sin = Math.sin(radAngle);

              scatterOffset.x = perpendicular.x * _cos - perpendicular.z * _sin;
              scatterOffset.z = perpendicular.x * _sin + perpendicular.z * _cos;
            }

            scatterOffset.multiplyScalar(scatterDist);
            var randomOffset = new Vec3((Math.random() - 0.5) * scatterDist * this.randomness * 2, (Math.random() - 0.5) * heightOffset * this.randomness * 0.5, (Math.random() - 0.5) * scatterDist * this.randomness * 2);
        ...
        getPerpendicularVector(vec) {
          if ((_crd && LayerManager === void 0 ? (_reportPossibleCrUseOfLayerManager({
            ...
            var perp = new Vec3(-vec.z, 0, vec.x);
            perp.normalize();
            return perp;
          }
        }
        ...
        move(dt) {
          var t = dt * this.jumpSpeed * 2.5;

          if (this.curveRangeSpeed) {
            var cy = this.curveRangeSpeed.curve.evaluate(this._time);
            t += t * cy;
            t = Math.max(0.01, t);
          }
          ...
          var pos = this.cubicBezier(this.controlPoints, this._time);

          if (this.curveRange) {
            var scaleMultiplier = (_crd && LayerManager === void 0 ? (_reportPossibleCrUseOfLayerManager({
              error: Error()
            }), LayerManager) : LayerManager).instance.SceneType === (_crd && SceneType === void 0 ? (_reportPossibleCrUseOfSceneType({
              error: Error()
            }), SceneType) : SceneType).D2 ? 256 : 4;

            var _cy = this.curveRange.curve.evaluate(this._time) * scaleMultiplier;

            var scale = 1 - Math.abs(this._time - 0.5) / 0.5;
            pos.y += _cy * scale;
          }
        ...
        cubicBezier(points, t) {
          ...
          var mt3 = mt2 * mt;
          var result = new Vec3();
          var temp0 = points[0].clone().multiplyScalar(mt3);
          result.add(temp0);
          var temp1 = points[1].clone().multiplyScalar(3 * mt2 * t);
          result.add(temp1);
          var temp2 = points[2].clone().multiplyScalar(3 * mt * t2);
          result.add(temp2);
          var temp3 = points[3].clone().multiplyScalar(t3);
          result.add(temp3);
          return result;
        }
...
//# sourceMappingURL=4103e3b8e8be7d1b0e7a8a67f28a2c29fe9aa2dc.js.map