  ...
  return {
    ...
    execute: function () {
      ...
      _export("GroupBag3D", GroupBag3D = (_dec = ccclass('GroupBag3D'), _dec2 = property({
        ...
        get NODECOUNT() {
          var count = this.count + this.propCount;
        ...
        getPropPlacePos(count) {
          ...
          var layer = Math.floor(count / this.layerCount);
          var layerY = layer * this.layerHeight;
          var layerCount = count % this.layerCount;
          var rx = Math.floor(layerCount % this.horizontal);
          var ry = Math.floor(layerCount / this.horizontal);

          if (this.sceneType == (_crd && SceneType === void 0 ? (_reportPossibleCrUseOfSceneType({
            error: Error()
          }), SceneType) : SceneType).D2) {
            var dx = this.startPos.x + rx * this.horizontalOffset.x + ry * this.verticalOffset.x;
            var dy = this.startPos.y + rx * this.horizontalOffset.y + ry * this.verticalOffset.y + layerY;
            this.tempV3.add3f(dx, dy, 0);
          } else {
            var _dx = this.startPos.x + rx * this.horizontalOffset.x + ry * this.verticalOffset.x;

            var dz = this.startPos.z + rx * this.horizontalOffset.y + ry * this.verticalOffset.y;
            var y = layerY + this.startPos.y;
            this.tempV3.add3f(_dx, y, dz);
          }
        ...
        set addProp(prop) {
          if (this.showMaxCount == -1 || this.showMaxCount > this.propList.length) {
            ...
            var originalPos = new Vec3(this.tempV3.x, this.tempV3.y, this.tempV3.z);
        ...
        updateSpringPhysics(dt) {
          // 计算偏离目标的距离
          var displacement = this._currentBend - this._bendingProgress; // 弹簧力：胡克定律 F = -k × x

          var springForce = -this.springStiffness * displacement; // 阻尼力：F = -c × v
          // 临界阻尼系数 = 2 * sqrt(k)，实际阻尼系数 = 临界阻尼 × dampingRatio

          var criticalDamping = 2 * Math.sqrt(this.springStiffness);
          var dampingCoefficient = criticalDamping * this.dampingRatio;
          var dampingForce = -dampingCoefficient * this._bendVelocity; // 总力

          var totalForce = springForce + dampingForce; // 更新速度和位置（欧拉积分）
        ...
        applyBending() {
          ...
          for (var i = 0; i < this.propList.length; i++) {
            ...
            var layerIndex = Math.floor(i / this.layerCount); // 使用固定的基准层数计算比例（确保无论多少道具，每层的偏移量一致）

            var layerRatio = layerIndex / this.bendingBaseLayers; // 弯曲因子（二次曲线，顶部更明显）

            var bendingFactor = Math.pow(layerRatio, 2); // 计算偏移量（沿z轴方向）

            var offset = new Vec3();
            Vec3.multiplyScalar(offset, this._bendingDir, this._currentBend * bendingFactor * this.maxBendingDistance); // 获取原始位置并应用偏移

            var originalPos = this._originalPositions[i];
...
//# sourceMappingURL=27a08fdf3bb68eddad668477af13f3f798542737.js.map