    ...
    execute: function () {
      ...
      _export("default", Shopping_Super = (_dec5 = ccclass('Shopping_Super'), _dec6 = property(Sprite), _dec7 = property({
        ...
        init() {
          ...
          for (var item of this.shoppingItems) {
            item.init();
        ...
        get isUse() {
          // 检查是否还有未完成的道具且有预付款余额
          var hasRemaining = this.shoppingItems.some(item => item.prePaid > 0);
          var isUse = hasRemaining && this.loopCount != 0;
        ...
        update(dt) {
          ...
          if (this.progress && this._totalOriginal > 0) {
            ...
            var proportion = 1 - this._totalRemaining / this._totalOriginal;
        ...
        moneyAccount(money, propId) {
          var item = this.getItem(propId);

          if (!item || item.remaining <= 0) {
            return false;
          }

          var oldRemaining = item.remaining;
        ...
        initJumpTime(propId) {
          var item = this.getItem(propId);
          var original = (item == null ? void 0 : item.amount) || 0;
...
//# sourceMappingURL=d87351923699bbc0b3bf2e76b9d6665983f7b324.js.map