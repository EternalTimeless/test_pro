    ...
    execute: function () {
      ...
      _export("default", GroupPropTransform = (_dec = ccclass("GroupPropTransform"), _dec2 = property(CCFloat), _dec(_class = (_class2 = class GroupPropTransform extends (_crd && GroupBag3D === void 0 ? (_reportPossibleCrUseOfGroupBag3D({
        ...
        get placePropPos() {
          ...
          this.count++;
          var count = this.NullPosIndex + this.count - 1;
        ...
        set addProp(prop) {
          var count = this.NullPosIndex; // prop.tran.priority = count;
        ...
        get prop() {
          var index = this.getIsTakeIndex();

          if (index == -1) {
            return null;
          } else {
            this._takeTime = this.takeTimeInterval;
            var node = this.propList[index];
        ...
        get NullPosIndex() {
          var count = this.propList.length;
        ...
        PropDown(index) {
          ...
          for (var i = index; i < this.propList.length; i += this.layerCount) {
            var prop = this.propList[i];
        ...
        _update(dt) {
          ...
          if (this._conversionTime <= 0) {
            this._conversionTime = this.transformeTime;
            var prop = this.tranProp.shift(); // prop.propID = this.takeId;
            // prop

            var pos = prop.node.position;
            var index = this.propList.indexOf(prop);
            var newProp = (_crd && PropManager === void 0 ? (_reportPossibleCrUseOfPropManager({
              error: Error()
...
//# sourceMappingURL=90a9e4f2cedb77d4d3f61b59f6edbe41b0c49742.js.map