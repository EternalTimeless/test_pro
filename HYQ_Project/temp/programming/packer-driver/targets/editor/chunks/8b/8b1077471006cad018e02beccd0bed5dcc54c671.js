System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _export("vectorPower_v2", vectorPower_v2 = (self, target, power = 1, yOff = 0) => {
        vectorPower2(self.worldPosition, target.worldPosition, power, yOff);
        tempV2.set(tempV3.x, tempV3.y);
        return tempV2;
      });

      _export("vectorPower", vectorPower = (self, target, power = 1, yOff = 0) => {
        return vectorPower2(self.worldPosition, target.worldPosition, power, yOff);
      });

      _export("vectorPower2", vectorPower2 = (self, target, power = 1, yOff = 0, isY = false) => {
        tempV3.set(target);
        tempV3.y += yOff;
      ...
      _export("getPosRandomPos", getPosRandomPos = (startPos, r = 128, lockVector = Vec3.ZERO, disVector = Vec3.ONE) => {
        // 球体内均匀随机点：使用球坐标系
        ...
        let z = radius * Math.cos(phi); // 应用 lockVector：锁定某轴的正/负方向
      ...
      _export("isFacingTargetHorizontal_vec", isFacingTargetHorizontal_vec = (targetDirection, playerForward, maxAllowedAngle = 30, ignoreHeight = false) => {
      ...
      _export("CalculatePerpendicularPosition", CalculatePerpendicularPosition = (dir, r, count) => {
        // 定义一个数组，用于存储计算出的位置
        let ve3Arr = []; // 获取垂直于给定方向的向量

        let uw = getPerpendicularVectors(dir); // 定义角度

        let angle = 0; // 定义每个位置之间的角度间隔

        let offDeg = 360 / count; // 循环计算每个位置

        for (let i = 0; i < count; i++) {
          // 将角度转换为弧度
          let deg = angle * Math.PI / 180; // 更新角度

          angle += offDeg; // 计算x和y的值

          let x = Math.cos(deg);
          let y = Math.sin(deg); // 将x和y的值乘以垂直于给定方向的向量
      ...
      _export("getPerpendicularVectors", getPerpendicularVectors = dir => {
        ...
        if (dir == Vec3.ZERO) {
          ...
          let reference = Math.abs(Vec3.dot(dir, Vec3.UP)) < 0.99 ? Vec3.UP : Vec3.FORWARD; // 将给定向量和参考向量叉乘，得到向量u，并归一化
      ...
      _export("getPerpendicularVector", getPerpendicularVector = input => {
        ...
        const temp = tempV3; // 方法1：尝试与标准基向量叉乘
      ...
      */
      ...
      _export("isPointInCameraView", isPointInCameraView = (worldPos, camera) => {
        // 将世界坐标转换为屏幕坐标
        const screenPos = camera.worldToScreen(worldPos); // 获取摄像机视口参数（归一化比例）

        const viewport = camera.rect; // 获取屏幕实际分辨率（使用设计分辨率适配不同设备）

        const screenSize = screen.windowSize; // 计算视口实际像素范围（关键修正点）
        ...
        const viewportHeight = viewport.height * screenSize.height; // 判断屏幕坐标是否在视口范围内

        const isInViewport = screenPos.x >= viewportX && screenPos.x <= viewportX + viewportWidth && screenPos.y >= viewportY && // 坐标系原点在左下角，直接比较
      ...
      _export("getCirclePosition", getCirclePosition = (center, count, index, radius, axis = Vec3.UP) => {
        const angle = 2 * Math.PI / count * index;
        const pw = getPerpendicularVectors(axis.clone()); // pos = center + (u * cos(angle) + w * sin(angle)) * radius
        ...
        const pos = (_crd && PoolManager === void 0 ? (_reportPossibleCrUseOfPoolManager({
          error: Error()
      ...
      _export("getRandomPointInCollider", getRandomPointInCollider = (collider, yValue = 0, seed) => {
        const size = collider.size;
        const node = collider.node; // 在本地坐标系中生成随机偏移量（使用 tempV3）
