    ...
    execute: function () {
      ...
      _export("Lightning", Lightning = (_dec = ccclass("Lightning"), _dec2 = property({
        ...
        update(deltaTime) {
          ...
          let endPos = Vec3.ZERO; // 获取起始节点的世界坐标位置
        ...
        collectLinPos(startPos, destPos, displace) {
          ...
          if (displace < this.detail) {
            ...
            let midZ = (startPos.z + destPos.z) / 2; // 为中点添加随机偏移，形成闪电的弯曲效果

            midX += (Math.random() - 0.5) * displace;
            midY += (Math.random() - 0.5) * displace;
            midZ += (Math.random() - 0.5) * displace;
            let midPos = new Vec3(midX, midY, midZ); // 递归处理两段，位移强度减半
        ...
        setPos(endWorldPos, startPos) {
          ...
          const distance = Vec3.distance(this.laser.node.worldPosition, endWorldPos);
...
//# sourceMappingURL=e2118395860d446afdd7349c12bbbbd5ff0a8915.js.map