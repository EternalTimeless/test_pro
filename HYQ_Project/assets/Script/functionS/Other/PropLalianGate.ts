import { _decorator, Animation, AudioClip, CCBoolean, CCFloat, CCInteger, instantiate, Label, MeshRenderer, Node, ParticleSystem, Prefab, Tween, tween, v3, Vec3 } from 'cc';
...
export class PropLalianGate extends BattleTarget3D {
    ...
    private playFinishFeedback(): void {
        ...
        this.unschedule(this.hideFinishEffect);
        this.finishEffectNode.active = true;
        const particles = this.finishEffectNode.getComponentsInChildren(ParticleSystem);
        for (let i = 0; i < particles.length; i++) {
            particles[i].stop();
            particles[i].play();
        }
        const animations = this.finishEffectNode.getComponentsInChildren(Animation);
        for (let i = 0; i < animations.length; i++) {
            animations[i].stop();
            animations[i].play();
        }
        if (this.finishEffectDuration > 0) {
            this.scheduleOnce(this.hideFinishEffect, this.finishEffectDuration);
        }
    }
